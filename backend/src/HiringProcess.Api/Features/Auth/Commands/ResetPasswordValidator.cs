...
{
    public ResetPasswordValidator(ILocalizationService loc, ICurrentLanguageService currentLang)
    {
        RuleFor(x => x.Token)
            .NotEmpty().WithMessage(_ => loc.Get("auth.tokenRequired", currentLang.Language));
